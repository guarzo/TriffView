...
namespace TriffView.Shared;

// The ESI HTTP transport and its retry policy, shared by TriffFleets and TriffSkills.
// Extracted verbatim from TriffFleetsController.SendEsiAsync so the two tools cannot
// drift apart on retry behaviour. Behaviour is unchanged from the original; the only
// difference is that the HttpClient, serializer options, and User-Agent product token
// arrive as parameters instead of being read from the calling class's statics.
