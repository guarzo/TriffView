...
internal static class SkillPlanEvaluator
{
    ...
    // which is the defect this evaluator exists to avoid.
    ...
    private static QueueEntry? SmallestSufficientEntry(
        IReadOnlyList<QueueEntry> queue,
        int skillId,
        int requiredLevel)
    {
        ...
        {
            if (entry.SkillId != skillId || entry.FinishedLevel < requiredLevel)
            {
                continue;
            }
