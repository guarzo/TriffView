...
internal sealed class SkillQueueItem
{
    ...
    // ESI documents finish_date as optional: "Date on which training of the skill will
    // complete. Omitted if the skill queue is paused." A paused queue therefore yields
    // entries with no date at all, which is why QueueEntry.FinishDate is nullable and why
    // a plan whose requirements are all queued on a paused queue has no ETA to report.
...
internal static class EsiSkillMapper
{
    public static Dictionary<int, int> ToTrainedLevels(CharacterSkillsResponse? response)
    {
        ...
        foreach (var skill in response?.Skills ?? new List<CharacterSkill>())
        {
            ...
            // active_skill_level, deliberately, not trained_skill_level. An alpha clone can
            // have SP for level 4 in a skill the clone state caps at 2; trained_skill_level
            // reports the SP, active_skill_level reports what the character can actually use.
            // The whole tool answers "can this character fly X", so active is the correct one.
