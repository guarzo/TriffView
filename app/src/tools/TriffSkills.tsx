...
function parsePlanPreview(text: string): ImportPreview | null {
  ...
  return {
    count: order.length,
    lines: order.slice(0, 5).map((skillName) => `${skillName} ${levelLabel(levels.get(skillName) ?? 0)}`),
  };
}
...
function fillFor(entry: MatrixEntry | null, requirementCount: number): number | undefined {
  ...
  const missing = (entry.missingSkills || []).length;
...
export default function TriffSkills() {
  ...
  // Backlog D-5: every glyph was its own tab stop, so a thirty-by-forty matrix put
  // more than a thousand of them between the first cell and the next control. The
  // grid is one tab stop now - exactly one cell carries tabIndex 0 and the rest
  // carry -1 - and the arrows move the cursor inside it.
  //
  ...
  // anything below; this is for the sighted keyboard user the old tab chain failed.
  ...
  // A callback ref rather than useRef, because the grid is behind hasMatrix and the
  // characters arrive from the bridge after the first commit. A ref plus an effect
  // with empty deps runs once, while the node is still null, and never again - which
  // pinned gridWidth at 0 and sent every render down the rotated branch regardless of
  // what the names measured. State setters are stable, so this attaches exactly once,
